...
try {
    ...
    $kategoriList = $pdo->query("SELECT k.*, COUNT(b.id) as jumlah_barang FROM kategori k LEFT JOIN barang b ON k.id = b.kategori_id GROUP BY k.id ORDER BY k.nama_kategori")->fetchAll();
} catch (Exception $e) {
    $kategoriList = [];
}
?>
...
<body class="bg-gray-100">
            ...
            <nav class="p-4 space-y-1 overflow-y-auto" style="max-height: calc(100vh - 80px);">
                ...
                <?php if (hasRole(['admin', 'petugas'])): ?>
                    ...
                    <a href="../kategori/index.php"
                        class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all bg-catalina-50 text-catalina-700 font-medium hover:bg-gray-100 hover:text-catalina-700">
                ...
                <div class="flex items-center justify-between px-4 lg:px-6 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Kelola kategori barang</p>
                        </div>
                    </div>
            ...
            <div class="p-4 lg:p-6">
                ...
                <div class="bg-white rounded-2xl p-4 lg:p-6 border border-gray-200 mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Total <?= count($kategoriList) ?> kategori</p>
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                                ...
                                <?php if (empty($kategoriList)): ?>
                                    <tr>
                                        <td colspan="4" class="px-4 py-12 text-center text-gray-400"><i
                                                class="fas fa-tags text-4xl mb-3"></i>
                                    ...
                                    <?php foreach ($kategoriList as $k): ?>
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        class="w-10 h-10 bg-catalina-100 rounded-xl flex items-center justify-center">
                                                        <i class="fas fa-tag text-catalina-600"></i>
                                                    </div>
                                                    <span
                                                        class="font-medium text-gray-800"><?= htmlspecialchars($k['nama_kategori']) ?></span>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center"><span
                                                    class="px-2 py-1 bg-catalina-100 text-catalina-700 rounded-full text-xs font-medium"><?= $k['jumlah_barang'] ?>
                                                    barang</span></td>
                                            <td class="px-4 py-3 text-center">
                                                <button
                                                    onclick="editKategori(<?= $k['id'] ?>, '<?= htmlspecialchars($k['nama_kategori']) ?>', '<?= htmlspecialchars($k['deskripsi'] ?? '') ?>')"
                                                    class="p-2 text-catalina-600 hover:bg-catalina-50 rounded-lg"
                                                    title="Edit"><i class="fas fa-edit"></i></button>
                                                <button
                                                    onclick="confirmDelete(<?= $k['id'] ?>, '<?= htmlspecialchars($k['nama_kategori']) ?>')"
                    ...
                    <div class="px-4 py-3 bg-gray-50 border-t border-gray-200 text-sm text-gray-500">
                        Total: <strong><?= count($kategoriList) ?></strong> kategori
                    </div>
                </div>
            </div>
        </main>
        ...
        <div class="bg-white rounded-2xl p-6 max-w-md w-full transform transition-all">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-bold text-gray-800" id="formTitle">
                    <i class="fas fa-tag text-catalina-500 mr-2"></i>Tambah Kategori
                </h3>
            ...
            <form method="POST" id="kategoriForm">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama_kategori" id="formNama" required
    ...
    <script>
        ...
        function openModal() {
            document.getElementById('formTitle').innerHTML = '<i class="fas fa-tag text-catalina-500 mr-2"></i>Tambah Kategori';
        ...
        function editKategori(id, nama, deskripsi) {
            document.getElementById('formTitle').innerHTML = '<i class="fas fa-edit text-catalina-500 mr-2"></i>Edit Kategori';
