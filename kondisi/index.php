...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    try {
        $pdo = db();
        // Check if kondisi is used by any barang
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM barang WHERE kondisi_id = ?");
        $stmt->execute([$_GET['delete']]);
        if ($stmt->fetchColumn() > 0) {
            $error = 'Kondisi tidak bisa dihapus karena masih digunakan oleh data barang!';
        } else {
            $stmt = $pdo->prepare("DELETE FROM kondisi WHERE id = ?");
            $stmt->execute([$_GET['delete']]);
            $success = 'Kondisi berhasil dihapus!';
        }
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        $error = 'Nama kondisi wajib diisi!';
    } else {
        try {
            $pdo = db();
            if ($id) {
                $stmt = $pdo->prepare("UPDATE kondisi SET nama_kondisi = ?, deskripsi = ? WHERE id = ?");
                $stmt->execute([$nama, $deskripsi, $id]);
                $success = 'Kondisi berhasil diupdate!';
            } else {
                $stmt = $pdo->prepare("INSERT INTO kondisi (nama_kondisi, deskripsi) VALUES (?, ?)");
                $stmt->execute([$nama, $deskripsi]);
                $success = 'Kondisi berhasil ditambahkan!';
            }
        } catch (Exception $e) {
            $error = 'Gagal menyimpan kondisi!';
        }
    }
}
...
try {
    ...
    $kondisiList = $pdo->query("SELECT k.*, (SELECT COUNT(*) FROM barang b WHERE b.kondisi_id = k.id) as jumlah_barang FROM kondisi k ORDER BY k.nama_kondisi")->fetchAll();
} catch (Exception $e) {
    $kondisiList = [];
}
?>
...
<body class="bg-gray-100">
                ...
                <div class="flex items-center justify-between px-4 lg:px-6 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Kelola kondisi barang inventaris</p>
                        </div>
                    </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <?php if (empty($kondisiList)): ?>
                                    <tr>
                                        <td colspan="5" class="px-4 py-12 text-center text-gray-400"><i
                                                class="fas fa-clipboard-check text-4xl mb-3"></i>
                                    ...
                                    <?php foreach ($kondisiList as $k): ?>
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 text-center"><span
                                                    ...
                                                    <?= $k['jumlah_barang'] ?> barang
                                                </span></td>
                                            <td class="px-4 py-3 text-center">
                                                <button
                                                    onclick="editKondisi(<?= $k['id'] ?>, '<?= htmlspecialchars($k['nama_kondisi']) ?>', '<?= htmlspecialchars($k['deskripsi'] ?? '') ?>')"
                                                    class="p-2 text-catalina-600 hover:bg-catalina-50 rounded-lg"
                                                    title="Edit"><i class="fas fa-edit"></i></button>
                                                <button
                                                    onclick="confirmDelete(<?= $k['id'] ?>, '<?= htmlspecialchars($k['nama_kondisi']) ?>')"
        ...
        <div class="bg-white rounded-2xl p-6 max-w-md w-full transform transition-all">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-bold text-gray-800" id="formTitle">
                    <i class="fas fa-clipboard-check text-amber-500 mr-2"></i>Tambah Kondisi
                </h3>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="formDeskripsi" rows="3"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-catalina-500 focus:ring-2 focus:ring-catalina-100"
                            placeholder="Deskripsi kondisi (opsional)"></textarea>
                    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-6 max-w-md w-full text-center">
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-2">Hapus Kondisi?</h3>
    ...
    <script>
        ...
        function openModal() {
            document.getElementById('formTitle').innerHTML = '<i class="fas fa-clipboard-check text-amber-500 mr-2"></i>Tambah Kondisi';
            document.getElementById('kondisiForm').reset();
        ...
        function editKondisi(id, nama, deskripsi) {
            document.getElementById('formTitle').innerHTML = '<i class="fas fa-edit text-amber-500 mr-2"></i>Edit Kondisi';
