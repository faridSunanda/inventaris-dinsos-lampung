...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($kode) || empty($nama)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM barang WHERE kode_barang = ? AND id != ?");
            $stmt->execute([$kode, $id]);
            if ($stmt->fetchColumn() > 0) {
                $error = 'Kode barang sudah digunakan!';
            } else {
                $stmt = $pdo->prepare("UPDATE barang SET kode_barang = ?, nama_barang = ?, kategori_id = ?, lokasi_id = ?, jumlah = ?, kondisi_id = ?, keterangan = ? WHERE id = ?");
                $stmt->execute([$kode, $nama, $kategori ?: null, $lokasi ?: null, $jumlah, $kondisi_id, $keterangan, $id]);
        ...
        <main class="flex-1 lg:ml-64 min-w-0">
                ...
                <div class="flex items-center justify-between px-4 lg:px-6 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Update data barang</p>
                        </div>
                    </div>
            ...
            <div class="p-4 lg:p-6">
                ...
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
                    ...
                    <span class="text-gray-800">Edit: <?= htmlspecialchars($barang['nama_barang']) ?></span>
                </div>
                            ...
                            <div>
                                ...
                                <select name="kondisi_id"
                                    ...
                                    <?php foreach ($kondisiList as $kd): ?>
                                        <option value="<?= htmlspecialchars($kd['id']) ?>" <?= ($_POST['kondisi_id'] ?? '') == $kd['id'] ? 'selected' : '' ?>>
